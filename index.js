...
const prefix = '.';
const ownerNumber = ['94766002356']; // ඔයාගේ no එක දාන්න 🙄
...
async function connectToWhatsApp() {
    ...
    async function downloadSessionFromMegaAndRestart() {
        ...
        const sessdata = config.SESSION_ID.replace("JoZiyA-OFC-xMD=", "");
    ...
    conn.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('👆 Running... ');
           conn.newsletterFollow("120363401814154475@newsletter") // මේ jid එක මාරු කරන්න එපා මැනිකලාහ් 😇. පහල එකට ඔයාගෙ එක දාන්න
           conn.newsletterFollow("120363401814154475@newsletter") // මේකට ඔයාගේ channel jid එක දාන්න
           console.log(" CHANNEL FOLLOW ✅")
           let up = `JoZiyA-OFC-xMD connected successful ✅\n\nPREFIX: ${prefix}`; // වෙනස් කරන්නහ්
           conn.sendMessage("94766002356@s.whatsapp.net", { image: { url: `https://files.catbox.moe/96u4ui.jpg` }, caption: up }) // වෙනස් කරන්නහ්
...
if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_READ_STATUS === "true"){
      ...
      if (timeDiff <= maxTime) {
        ...
        try {
          ...
          let rep = `Status seen by Queen JoZiyA-OFC-xMD💙`; // වෙනස් කරගන්නන 
...
const reply = (teks) => {
    conn.sendMessage(from, {
        ...
        contextInfo: {
            ...
            externalAdReply: {
            // මේ ටිකම හදාගන්න 
                title: `JoZiyA-OFC-xMD`, 
                body: `The Best Multi Device Whatsapp Bot.`,
                thumbnailUrl: `https://files.catbox.moe/96u4ui.jpg`,
                mediaType: 1,
...
app.get("/", (req, res) => {
    res.send("hey, JoZiyA-OFC-xMD-KING started✅");
});
